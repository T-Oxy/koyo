...
def main():
    ...
    for flag in tqdm(flags, desc="flag"):
        ...
        for pref in tqdm(prefs, desc="pref"):
            ...
            sorted_soa = sorted(soa_of_word.items(), key=lambda x:(x[1], x[0]), reverse=True)

            result_dir = '/now24/naruse/koyo/result/related_words/'
            os.makedirs(result_dir, exist_ok=True)
            with open(result_dir + f'{flag}_{pref}_2pmi.txt', 'w') as f:
                for w, pmi in sorted_pmi:
                    f.write(f'{w}, {pmi}\n')
            os.makedirs(result_dir, exist_ok=True)
            with open(result_dir + f'{flag}_{pref}_2soa.txt', 'w') as f:
