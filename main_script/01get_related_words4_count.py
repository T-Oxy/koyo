...
sakura_tk = [pd.to_datetime(date) for date in pd.date_range(start="2014-03-25", end="2014-03-30", tz="Japan", freq="D")]
sakura_hk = [pd.to_datetime(date) for date in pd.date_range(start="2014-04-29", end="2014-05-01", tz="Japan", freq="D")]
sakura_is = [pd.to_datetime(date) for date in pd.date_range(start="2014-04-01", end="2014-04-07", tz="Japan", freq="D")]
...
koyo_tk = jumoku_tk = list(set(icho_tk + kaede_tk))
koyo_hk = jumoku_hk = list(set(icho_hk + kaede_hk))
koyo_is = jumoku_is = list(set(icho_is + kaede_is))

correct_dates = {
    "tk": {"icho": icho_tk, "kaede": kaede_tk, "sonota": koyo_tk, "koyo": koyo_tk, "jumoku": jumoku_tk, "sakura": sakura_tk},
    "hk": {"icho": icho_hk, "kaede": kaede_hk, "sonota": koyo_hk, "koyo": koyo_hk, "jumoku": jumoku_hk, "sakura": sakura_hk},
    "is": {"icho": icho_is, "kaede": kaede_is, "sonota": koyo_is, "koyo": koyo_is, "jumoku": jumoku_is, "sakura": sakura_is},
    }
