...
def read_relation_words(pref, flag):
    rwords_dir = "/now24/naruse/koyo/result/related_words/"
...
def count_rtweets(pref, db, flag, relation_words, fixed_num):
    ...
    return all_day_counts, fixed_num
...
def main():
    ...
    result_dir = '/now24/naruse/koyo/result/rtweets_count/'

    for pref, db in tqdm(dbs, desc="DB(pref)"):
        for flag in tqdm(flags, desc="flag"):
            relation_words = read_relation_words(pref, flag)
            for fixed_num in tqdm(fixed_nums, desc="fixed_num"):
                all_day_counts, fixed_num = count_rtweets(pref, db, flag, relation_words, fixed_num)
