...
icho_tk = [pd.to_datetime(date) for date in pd.date_range(start="2015-11-30", end="2015-12-11", tz="Japan", freq="D")]
icho_hk = [pd.to_datetime(date) for date in pd.date_range(start="2015-11-02", end="2015-11-12", tz="Japan", freq="D")]
icho_is = [pd.to_datetime(date) for date in pd.date_range(start="2015-11-04", end="2015-11-10", tz="Japan", freq="D")]

kaede_tk = [pd.to_datetime(date) for date in pd.date_range(start="2015-12-04", end="2015-12-12", tz="Japan", freq="D")]
kaede_hk = [pd.to_datetime(date) for date in pd.date_range(start="2015-10-29", end="2015-11-29", tz="Japan", freq="D")]
kaede_is = [pd.to_datetime(date) for date in pd.date_range(start="2015-11-22", end="2015-12-03", tz="Japan", freq="D")]
...
def main():
    ...
    for flag in tqdm(flags, desc="flag"):
        ...
        for pref, db in tqdm(dbs, desc="DB(pref)"):
            ...
            result_dir = '/now24/naruse/koyo/result/related_words2015/'
